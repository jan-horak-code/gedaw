...
mininfo= function (vec, h=10) {

# comments:
	# computes h number of minimal values and returns dataframe with those values and their positions in a vector
...
# arguments:
	...
	# h: Howmany, i.e. how many minimal values shoud be returned
...
# all the function it this else
	
		{
			...
			for (i in sequen)
				{
					orderi = sequen[i]
					mini = min(vec[-positions], na.rm=T)
					posi = which(vec==mini)
					counti = length(posi)
	
					orders = append(orders, rep(sequence[sequen[i]], length.out=counti))
					value = append(value, rep(mini, length.out=counti))
